...
function verifiedBadge(user) {
  ...
  return `<svg class="verified-badge" viewBox="0 0 22 22" width="16" height="16"><path fill="#1d9bf0" d="M20.396 11c-.018-.646-.215-1.275-.57-1.816-.354-.54-.852-.972-1.438-1.246.223-.607.27-1.264.14-1.897-.131-.634-.437-1.218-.882-1.687-.47-.445-1.053-.75-1.687-.882-.633-.13-1.29-.083-1.897.14-.273-.587-.704-1.086-1.245-1.44S11.647 1.62 11 1.604c-.646.017-1.273.213-1.813.568-.54.354-.972.853-1.246 1.44-.607-.223-1.264-.272-1.897-.14-.634.13-1.217.436-1.687.882-.445.469-.75 1.053-.882 1.687-.13.633-.084 1.29.14 1.897-.586.274-1.084.706-1.438 1.246-.354.54-.552 1.17-.57 1.816.02.647.218 1.276.572 1.817.354.54.852.972 1.438 1.245-.224.607-.271 1.264-.14 1.898.13.634.435 1.217.88 1.687.47.445 1.054.75 1.688.881.633.131 1.29.084 1.897-.14.274.587.705 1.086 1.246 1.44.54.354 1.167.551 1.813.569.647-.02 1.276-.216 1.817-.57.54-.353.972-.852 1.245-1.438.607.223 1.264.27 1.897.14.634-.131 1.217-.436 1.687-.881.445-.47.75-1.053.881-1.687.131-.634.084-1.291-.14-1.898.586-.273 1.084-.705 1.437-1.245.354-.541.552-1.17.57-1.817zM9.662 14.85l-3.429-3.428 1.293-1.302 2.072 2.072 4.4-4.794 1.347 1.246z"/></svg>`;
}
...
async function hydratePosts(rows) {
  ...
  let originals = {};
  if (repostIds.length) {
    const { data } = await supabaseClient.from('posts').select('*, profiles!posts_user_id_fkey(*)').in('id', repostIds);
    (data || []).forEach(o => originals[o.id] = o);
  }
  let parents = {};
  if (replyToIds.length) {
    const { data } = await supabaseClient.from('posts').select('id, profiles!posts_user_id_fkey(username)').in('id', replyToIds);
...
async function fetchTimeline(sort) {
  ...
  const { data, error } = await supabaseClient.from('posts').select('*, profiles!posts_user_id_fkey(*)')
...
async function fetchThread(postId) {
  const { data: postRow, error } = await supabaseClient.from('posts').select('*, profiles!posts_user_id_fkey(*)').eq('id', postId).single();
  if (error) throw new Error('المنشور غير موجود');
  const { data: replyRows } = await supabaseClient.from('posts').select('*, profiles!posts_user_id_fkey(*)').eq('reply_to', postId).order('created_at', { ascending: true });
...
async function fetchProfilePage(username) {
  ...
  const { data: postRows } = await supabaseClient.from('posts').select('*, profiles!posts_user_id_fkey(*)').eq('user_id', user.id).is('reply_to', null).order('created_at', { ascending: false }).limit(50);
...
async function searchAll(q) {
  ...
  const { data: postRows } = await supabaseClient.from('posts').select('*, profiles!posts_user_id_fkey(*)').ilike('content', `%${q}%`).order('created_at', { ascending: false }).limit(30);
  const posts = await hydratePosts(postRows || []);
...
function switchTab(view) {
  ...
  $('#fab-compose').classList.toggle('hidden', view === 'timeline');
}

$('#fab-compose').addEventListener('click', () => {
  switchTab('timeline');
  loadView('timeline');
  setTimeout(() => $('#composer-text').focus(), 100);
});
  ...
  <article class="post" data-id="${display.id}" data-author-id="${display.author.id}">
    ...
    <div class="post-main">
      ...
      <div class="post-header">
        <span class="name">${escapeHtml(display.author.display_name)}${verifiedBadge(display.author)}</span>
    ...
    <div class="profile-header">
      ...
      <div class="profile-info">
        ...
        <div class="profile-name-row">
          <div>
            <p class="profile-name">${escapeHtml(data.user.display_name)}${verifiedBadge(data.user)}</p>
      ...
      if (data.users.length) {
        ...
        html += data.users.map(u => `
          <div class="post" data-user="${escapeHtml(u.username)}" style="cursor:pointer;">
            ...
            <div class="post-main">
              <div class="post-header"><span class="name">${escapeHtml(u.display_name)}${verifiedBadge(u)}</span></div>
